...
Classify every top-level key and every `packageRules` entry in the target
repo's config **exactly once**. One classification, one disposition.
...
| **fragmenting** | A narrow local group that a shared broad group now already covers. Once the shared group unifies those deps, the local group re-splits them. | Delete, or fold several overlapping local groups into one focused (e.g. risk-based) group. |
