...
| **fragmenting** | A narrow local group that a shared broad group now already covers. Once the shared group unifies those deps, the local group re-splits them. | Delete the *grouping* — but keep any option the shared rule doesn't set, moving it to a rule of its own. Or fold several overlapping local groups into one focused (e.g. risk-based) group. |
